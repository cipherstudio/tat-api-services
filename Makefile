...
.PHONY: help \
        ...
        docker-migration-run docker-migration-generate
...
ifeq ($(OS),Windows_NT)
	...
	@$(PACKAGE_MANAGER) run migration:create --name=$$env:name
else
	@if [ -z "$(name)" ]; then echo "Error: Migration name is required." && exit 1; fi
	@$(PACKAGE_MANAGER) run migration:create --name=$(name)
endif

migration-generate: ## Generate a migration from entity changes. Usage: make migration-generate name=<migration-name>
ifeq ($(OS),Windows_NT)
	@if (!$$env:name) { Write-Host "Error: Migration name is required." -ForegroundColor Red; exit 1 }
	@$(PACKAGE_MANAGER) run migration:generate --name=$$env:name
else
	@if [ -z "$(name)" ]; then echo "Error: Migration name is required." && exit 1; fi
	@$(PACKAGE_MANAGER) run migration:generate --name=$(name)
endif
...
docker-migration-generate: ## Generate database migration in Docker container
	docker-compose exec api yarn migration:generate --name=$(name) 