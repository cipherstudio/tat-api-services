...
export class OfficeDomesticRepository extends KnexBaseRepository<OfficeDomestic> {
  ...
  async findById(id: number): Promise<OfficeDomestic | undefined> {
    ...
    return dbEntity ? await toCamelCase<OfficeDomestic>(dbEntity) : undefined;
  }
  ...
  async find(conditions: Record<string, any> = {}): Promise<OfficeDomestic[]> {
    ...
    return Promise.all(dbEntities.map(async (e) => await toCamelCase<OfficeDomestic>(e)));
  }
  ...
  async findWithPagination(
    ...
    return {
      ...result,
      data: await Promise.all(result.data.map(async (e) => await toCamelCase<OfficeDomestic>(e))),
    };
  }
  ...
  async findWithPaginationAndSearch(
    ...
    return {
      data: await Promise.all(data.map(async (e) => await toCamelCase<OfficeDomestic>(e))),
      meta: {
        total,
        page,
        limit,
