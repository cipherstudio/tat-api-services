...
export class OfficeInternationalRepository extends KnexBaseRepository<OfficeInternational> {
  ...
  async findById(id: number): Promise<OfficeInternational | undefined> {
    ...
    return dbEntity ? await toCamelCase<OfficeInternational>(dbEntity) : undefined;
  }
  ...
  async find(conditions: Record<string, any> = {}): Promise<OfficeInternational[]> {
    ...
    return Promise.all(dbEntities.map(async (e) => await toCamelCase<OfficeInternational>(e)));
  }
  ...
  async findWithPagination(
    ...
    return {
      ...result,
      data: await Promise.all(result.data.map(async (e) => await toCamelCase<OfficeInternational>(e))),
    };
  }
  ...
  async findWithPaginationAndSearch(
    ...
    return {
      data: await Promise.all(data.map(async (e) => await toCamelCase<OfficeInternational>(e))),
      meta: {
        total,
        page,
        limit,
