import { Injectable } from '@nestjs/common';
import { OpHeadT, OpHeadTPaginate } from '../entities/op-head-t.entity';
import { KnexBaseRepository } from '../../../common/repositories/knex-base.repository';
import { KnexService } from '../../../database/knex-service/knex.service';
import { toCamelCase } from '../../../common/utils/case-mapping';
import { QueryOpHeadTDto } from '../dto/query-op-head-t.dto';

@Injectable()
export class OpHeadTRepository extends KnexBaseRepository<OpHeadT> {
  constructor(knexService: KnexService) {
    super(knexService, 'OP_HEAD_T');
  }

  async findWithQuery(query: QueryOpHeadTDto): Promise<OpHeadTPaginate> {
    const conditions: Record<string, any> = {};
    if (query.phtCode) conditions['PHT_CODE'] = query.phtCode;
    if (query.phtNameT) conditions['PHT_NAME_T'] = query.phtNameT;

    let builder = this.knex(this.tableName).where(conditions);
    if (query.limit !== undefined) builder = builder.limit(query.limit);
    if (query.offset !== undefined) builder = builder.offset(query.offset);

    // นับจำนวนทั้งหมด (ไม่ใส่ limit/offset)
    const countQuery = this.knex(this.tableName).where(conditions);
    const countResult = await countQuery.count('* as count').first();
    const total = Number(countResult?.count || 0);

    const dbEntities = await builder.select();
    const data = await Promise.all(
      dbEntities.map(async (e) => await toCamelCase<OpHeadT>(e)),
    );
    return {
      data,
      meta: {
        total,
        limit: query.limit ?? 10,
        offset: query.offset ?? 0,
      },
    };
  }
}
